...
ob_start(); ?>
  ...
  $output .= '<div id="kad-wp-gallery'.$gallery_id.'" class="kad-wp-gallery init-mosaic-isotope kad-light-wp-gallery clearfix" data-fade-in="'.$animate.'" data-iso-selector=".g_item" data-iso-style="packery" data-iso-filter="false">';
    if ($columns == '3') {
...
if(!empty($lightboxsize)) {$attachmentsize = $lightboxsize;} else {$attachmentsize = 'full';} 
  ...
  foreach ($attachments as $id => $attachment) {
    ...
    $output .= '<div class="'.$mosaic_itemsize.' g_item"><div class="grid_item kt_item_fade_in kad_gallery_fade_in gallery_item g_mosiac_item">';
        ...
        $output .= '<a href="'.$attachment_url.'" rel="lightbox[pp_gal]" class="lightboxhover">';
          if (trim($attachment->post_excerpt) && $caption == 'true') {
